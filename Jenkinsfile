    ...
    stages {
        ...
        stage("deploy") {
            steps {
                ...
                sh '''
                ansible staging_servers -m shell -a "sudo docker pull 0686519782/nginx-test:1.0.$BUILD_NUMBER"
                ansible staging_servers -m shell -a "sh kill.sh"
                ansible staging_servers -m shell -a "sh docker_rm.sh"
                ansible staging_servers -m shell -a "sudo docker run -d --name nginx-t01 -p 8090:80 0686519782/nginx-test:1.0.$BUILD_NUMBER"
                '''
            }
        }
    }
